...
pub async fn get_cowswap_cost(usdc_sell_amount: u128) -> Result<(f32, f32)> {
    ...
    let cowswap_costs: f32 = v.quote.fee_amount.parse()?;
    let output: f32 = v.quote.buy_amount.parse()?;
    Ok((cowswap_costs / 1000000f32, output))
}
